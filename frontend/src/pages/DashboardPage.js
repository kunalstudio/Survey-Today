...
const DashboardPage = () => {
  ...
  const handleClose = async (id) => {
    ...
    try {
      ...
      flash(err.response?.data?.message || 'Failed to close', true);
    }
  };
  ...
  const handleDuplicate = async (id) => {
    try {
      ...
      flash(err.response?.data?.message || 'Failed to duplicate', true);
    }
  };
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          <h1>Dashboard</h1>
          <p className="text-muted">Welcome back, {user?.name} 👋 Here is your workspace summary.</p>
        </div>
      ...
      {/* Render-Style Metric Stat Cards */}
      <div className="stats-grid">
        ...
        <div className="stat-card stat-card-purple">
          <div className="stat-icon">📊</div>
          <div className="stat-value" style={{ color: '#c084fc' }}>{stats.totalResponses}</div>
      ...
      <div className="section">
        ...
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <Link to="/surveys/new" className="btn btn-render-white">Create your first survey</Link>
          </div>
        ) : (
              ...
              <div key={survey._id} className="survey-card">
                <div className="survey-card-top">
                  ...
                  {survey.description && (
                    <p className="survey-card-desc">
                      {survey.description.slice(0, 110)}{survey.description.length > 110 ? '…' : ''}
                    </p>
                  )}
                ...
                <div className="survey-card-actions">
                  ...
                  {survey.status === 'draft' && survey.questions?.length > 0 && (
                    <button className="btn btn-sm btn-indigo" onClick={() => handlePublish(survey._id)}>🚀 Publish</button>
                  )}
